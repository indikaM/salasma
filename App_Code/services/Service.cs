...
public class Service
{
    ...
    public int SaveTask(Task task){
        ...
        try{
           ...
           logger.Debug(SQL);
           id = _db.Execute(SQL);
           var Result = _db.QuerySingle("select last_insert_id() 'id' ");
           id = (int)Result.id;
           logger.Debug(id);
        }catch(Exception e){
            logger.Debug("Test data..."+e.Message);
            FreeConnection(); 
        }
        finally{
            FreeConnection();
    ...
    public UserProjects GetUserProject(int userID,int projectID){
        ...
        List<Project> ProjectList = new List<Project>();
        String _SQL = "select ID,UserID,ProjectID from userprojects "+
              "where UserID='"+userID+"' and ProjectID='"+projectID+"'";
        try{
          GetConnection();
          var Results = _db.Query(_SQL);
          foreach(var result in Results){
